...
class PIDRateEstimatorSuite extends TestSuiteBase with Matchers {

    test("first bound is None"){
      val p = new PIDRateEstimator(-1D, 0D, 0D)
    ...
    test("bound is never negative"){
      val p = new PIDRateEstimator(-1D, -1D, 0D)
      ...
      val res = for (i <- List.range(0, 50)) yield
        p.compute(times(i), elements(i), proc(i), sched(i))
    ...
    test("with some accumulated and some positive error, |I| > 0, stay below the processing speed"){
      val p = new PIDRateEstimator(-1D, -.01D, 0D)
      val times = List.tabulate(50)(x => x * 20) // every 20ms
      val rng = new Random()
