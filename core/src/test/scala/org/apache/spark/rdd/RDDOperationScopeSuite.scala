...
class RDDOperationScopeSuite extends FunSuite with BeforeAndAfter {
  ...
  test("withScope") {
    ...
    RDDOperationScope.withNamedScope(sc, "scope1", allowNesting = false) {
      rdd1 = new MyCoolRDD(sc)
      RDDOperationScope.withNamedScope(sc, "scope2", allowNesting = false) {
        rdd2 = new MyCoolRDD(sc)
        RDDOperationScope.withNamedScope(sc, "scope3", allowNesting = false) {
  ...
  test("withScope with partial nesting") {
    ...
    var rdd2: MyCoolRDD = null
    var rdd3: MyCoolRDD = null
    RDDOperationScope.withNamedScope(sc, "scope1", allowNesting = true) { // allow nesting here
      rdd1 = new MyCoolRDD(sc)
      RDDOperationScope.withNamedScope(sc, "scope2", allowNesting = false) { // stop nesting here
        rdd2 = new MyCoolRDD(sc)
        RDDOperationScope.withNamedScope(sc, "scope3", allowNesting = false) {
          rdd3 = new MyCoolRDD(sc)
        }
  ...
  test("withScope with multiple layers of nesting") {
    ...
    var rdd2: MyCoolRDD = null
    var rdd3: MyCoolRDD = null
    RDDOperationScope.withNamedScope(sc, "scope1", allowNesting = true) {
      rdd1 = new MyCoolRDD(sc)
      RDDOperationScope.withNamedScope(sc, "scope2", allowNesting = true) {
        rdd2 = new MyCoolRDD(sc)
        RDDOperationScope.withNamedScope(sc, "scope3", allowNesting = true) {
          rdd3 = new MyCoolRDD(sc)
        }
      }
    }
